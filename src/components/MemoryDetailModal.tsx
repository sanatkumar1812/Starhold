...
export const MemoryDetailModal = ({
  ...
  useEffect(() => {
    if (isOpen) {
      if (showAnimation && memory?.is_unlocked) {
  ...
  }, [isOpen, memory?.is_unlocked, showAnimation]);
      ...
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] bg-gradient-to-b from-background via-background to-background/95 backdrop-blur-2xl border-primary/20 p-0 overflow-hidden shadow-2xl shadow-primary/5">
          ...
          <ScrollArea className="max-h-[90vh]">
            <div className={`relative p-8 pt-12 space-y-8 transition-opacity duration-500 ${!animationComplete ? 'opacity-0' : 'opacity-100'
              }`}>
              {/* Star Map Section */}
